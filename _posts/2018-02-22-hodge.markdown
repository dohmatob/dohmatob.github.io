...
Here, $\Omega^k(M)$ denotes the vector space of all [differential $k$-forms][4] on $M$. It is the span of $k$-fold *wedge products* of the form $dx_{i_1} \wedge \ldots \wedge dx_{i_k}$ whose coefficients are smooth functions on $M$. $\Omega^0(M)$ is simply the ring of $\mathcal C^\infty$ real-valued functions on $M$. For $k \ne 0$, $\Omega^k(M)$ has the structure of a $\mathcal C^\infty(M)$-module of rank $\mathcal C^\infty(M)$, the ring of smooth functions on $M$. It's not hard to see that  $\Omega^k(M)$ has (algebraic) rank ${n\choose k}$, since this number if precisely counts how many unique ways one can form the products $dx_{i_1} \wedge \ldots \wedge dx_{i_k}$, modulo the ordering of the terms.
