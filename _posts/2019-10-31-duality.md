...
Finally, the *dual cone* of $C$ is the subset of $C^\star$ of linear forms which are nonnegative on $C$, i.e

$$
C^\star := \{\xstar \in \Xstar \mid \langle \xstar,x\rangle \ge 0,\;\forall x \in C\}.
$$

Notice that $C^\star$ and $N_C(x)$ are *convex cones* of $\Xstar$.
...
=================================================
We motivate things with the Linear Programming (LP), a sub-field of convex optimization which basically gave birth to convex optimization and convex convex analysis (thanks to Lionid Kantorovich, R.T Rockafellar, Jean-Jaques Moreau, etc.). So, consider the case where $\Y$ be a real Banach space, and $\X$ and $A$ be as before. Let $b \in \Y$, $c \in \Xstar$, $C \subseteq \Y$,
and consider the problem

$$
p = \inf_{Ax - b \in C}\langle c, x\rangle \tag{LP}.
$$

It's well-know that the "dual" of the above problem is the problem

$$
d=\sup_{\ystar \in \Ystar,\; \ystar \in C^\star,\;\Astar\ystar = c}\langle \ystar, b\rangle, \tag{Dual-LP}
$$
...
&=
\begin{cases}
\langle \ystar, b\rangle,&\mbox{ if } -\ystar \in C^\star,\\
+\infty,&\mbox{ else.}
\end{cases}
\end{split}
$$
