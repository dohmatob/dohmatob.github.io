...
Finally, the *polar cone* of $C$ is the subset $C^\circ$ of linear forms on $\X$ which are nonpositive on $C$,
i.e
...
---
***Lemma 2.2.4 (subdifferential of indicator funciton).***
...
*Hint.* Noting that $$\|.\|$$ is the support function of the unit ball in the dual space $\X$, invoke Theorem 2.4.2.
...
Moreover, if $C$ is a cone, then $\chi_C^\star = \chi_{C^\circ}$.
...
$$
\chi_C(\xstar) := \sup_{x \in \X}\;\langle \xstar,x\rangle - \chi_C(x) = \sup_{x \in \X}\;\langle \xstar,x\rangle =: \sigma_C(\xstar).
$$

Now, suppose $C$ is a cone. If $\xstar \in C^\circ$, then $\langle \xstar,x\rangle \le 0$ for all $x \in C$ and this lower bound is attained, since $0 \in C$ as $C$ is a cone. Thus $\chi_C(\xstar) = \sup_{x \in C}\;\langle \xstar,x\rangle = 0$. One the other hand, if $\xstar \not \in C^\star$, then there exists $x_0 \in C$ such that $\langle \xstar,x_0\rangle > 0$.
Since the ray $$R_\lambda(x_0) := \{\lambda x_0 \mid \lambda \ge 0\}$$ is contained in $C$ (as the latter is a cone),
it follows that

$$
\chi_C(\xstar) = \sup_{x \in C}\;\langle \xstar,x\rangle \ge \sup_{x \in R_\lambda}\;\langle \xstar,x\rangle = \lambda \langle \xstar,x_0\rangle \overset{\lambda \rightarrow +\infty}{\longrightarrow} +\infty, \text{ because } \langle \xstar,x_0\rangle > 0.
$$

There $\chi_C^\star = \chi_{C^\circ}$ as claimed. $\qed$
...
$\sigma_C$ and the indicator function $\chi_C$ are Fenchel conjugates of each other. Use this to provide an alternative answer
to Exercise 2.4.1.
...
&=
\begin{cases}
\langle \ystar, b\rangle,&\mbox{ if }\ystar \in C^\circ\\
+\infty,&\mbox{ else,}
\end{cases}
\end{split}
$$
...
the fact that $-C^\circ = C^\star$, we have
