...
**Exercise 1.2.1.** Let $\X$ and $\Y$ be abstract sets, and consider a function
$\Phi: \X \times \Y \rightarrow (-\infty,+\infty]$. Prove the following inequality

$$
\inf_x\;\sup_y\; \Phi(x, y) \ge \sup_y\;\inf_x\; \Phi(x, y).
$$
...
==================================================================
Given a function $g: \X \rightarrow (-\infty, +\infty]$, it's *effective domain* denoted $\dom(g)$ is the subset of $\X$ on which $g$ takes finite values, i.e $$\dom(g) := \{x \in X \mid g(x) < +\infty\}.$$ The *subdifferential* of $g$ at a point $x \in \X$, denoted $\partial g(x)$ is defined by
...
Then we have the following charaterization

***Fermat's Rule.***
 $x \in \opt(g) \iff 0 \in \partial g(x).$

*Proof.* $x \in \opt(g) \iff g(x') \ge g(x)\; \forall x' \in \X \iff g(x') \ge g(x) + \langle 0,x'-x\rangle\; \forall x' \in \X \iff 0 \in \partial g(x).\;\qed$
...
II.4 Exercises
==============
Except otherwise stated, $\X$ is a (reflexive) Banach space over $\mathbb R$ or $\mathbb C$.
...
**Exercise 2.4.2. (basic properties of Fenchel conjugates.)** Given a function $g:\X \rightarrow (-\infty, +\infty]$ and scalar $\lambda \ne 0$ and a point $a \in \X$, prove that:
...
**Exercise 2.4.5 (entropy).** Let $a \in \mathbb R^n$ and $\sigma_1,\ldots,\sigma_n > 0$, and consider the function $g:\mathbb R^n \rightarrow (-\infty, +\infty]$ defined by
$$
g(x)=
\begin{cases}
...
**Exercise 2.4.6 (indicator and support functions).** Let $C$ be a non-empty subset of $\X$. Prove that the support function
...
we already showed at the end of section II.3 that
