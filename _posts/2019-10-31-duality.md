...
*Proof.*
Note that $$\partial g_1 + \partial g_2 := \{v_1 + v_2 \mid (v_1, v_2) \in \partial g_1 \times \partial g_2\}.$$
So, if $v = v_1 + v_2 \in \partial g_1 + \partial g_2$ and $x' \in \X$, then by the definition of subdifferentials, we have
...
Also note that $\partial_y \Phi(x, y) = \sum_{i=1}^nx_i\partial f(y)$ (Minkowski sum of sets). Now invoke Theorem 2.2.2 and conclude. $\qed$
...
The *Fenchel conjugate* (aka *convex conjugate*) of a function $g:\X \rightarrow (-\infty, +\infty]$ is the function $\gstar:\Xstar \rightarrow (-\infty, +\infty]$ defined by
