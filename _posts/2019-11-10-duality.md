...
$\phi(\lambda x) = |\lambda|\phi(x)$ for all $(x,\lambda) \in \X \times \mathbb R$.


**Exericse 1.3.8.** Let $C$ be a nonempty convex set of a real normed vector space $\X$. Prove that

$$
\inf_{x \in C}\xstar(x) \le \|\xstar\|_*\dist(0, C),\;\forall \xstar \in \Xstar,
$$

where $$\dist(0, C) := \inf_{x \in C}\|x\|$$ is the distance of the set $C$ from the origin. Moreover, prove that if $C$ is convex,
then
