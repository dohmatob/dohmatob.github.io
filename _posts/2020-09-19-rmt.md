...
some moment conditions) is a *Tracy-Widom* in the limit when $n,N \to
...
>**Theorem.** Let $N$ and $n$ be positive integers with $n/N =:
...
random matrix of the type in the above Theorem, have smallest
singular-value of size $\Omega(1)\sqrt{N}$.

>**Corollary.** Let the random matrix $A$ be as in the previous
   theorem. For every $p \in (\lambda, 1]$, there exists $c > 0$
   depending only on $\lambda$ and $p$ such that w.p $1-o(1)$, it
   holds that $s_{\min}(A_I) \ge c\sqrt{N}$ for every $I \subseteq
   [N]$ with $|I| \ge pN$.

*Proof or corollary.* Let $C>0$ be arbitrary. Denote by $\mathcal I_k$
...
Now, let $I \in \mathcal I$ with $|I| = k \ge pN$. Because $A_I$ has aspect
ratio $n/k \le n/(pN) = \lambda/p \in (0, 1)$, we may apply the
previous theorem to guarantee the existence of $c_k>0$ depending only
...
**Exercise 1 (Entropy and counting).** Prove the bound (1). Hint: Make connection to binomial distribution.

**Exercise 2 (The "union bound").** Prove that if $E_1,\ldots,E_k$ are events on the same
  space, then $\bP(\cup_{i=1}^k E_i) \le k\max_{i=1}^k \bP(E_i)$.

**Exercise 3 (Volume-packing).** Prove that
the number of balls in $\mathbb R^k$ of radius $1/2$ which can
be enclosed in $\mathbb S_{k-1}$ is at most $6^k$.

## II -- Main ingredients

Let $(\mathcal X,d)$ be a metric space, $S \subseteq \mathcal X$, and
...
>**Lemma (covering-number bound).** For every $\epsilon \in (0, 1)$,
we have the bound $| \enet(S)| \le (3/\epsilon)^k$. More generally, if
...
argument and is left as an exercise (Hint: workout **Exercise 3** of the
previous section).
...
>**Fact 1 (Gaussian small-ball probability).** *If $X =
...
*Proof of the main claim.* Let $\epsilon \in (0, 1)$, to be prescribed
 ...
 $\mathbb S_{n-1}$. Note that $| \enet | \le (3/\epsilon)^n =
