---
layout: post
title:  "A criminal's view on random matrix theory"
...
Since $A$ is a random matrix, both  $s_{\min}(A)$ and $s_{\max}(A)$ are random variables. We will be interested in non-asymptotic confidence intervals for these quantities. The main result of this post will be the following.

>**Theorem.** Let the entries of $A$ be iid copies of a subGaussian random variable. Then, there exists an absolute constant $\lambda_0 \in (0, 1)$ such that if $\lambda < \lambda_0$, then there exists constants $c,C>0$ which only depend on $\lambda$ such that $s_{\min}(A) \ge c$ w.p $1-2e^{-CN}$.
...
$A>0$ such that
$$
\mathbb P(|\zeta| \ge t) \le Ae^{-t^2/(2\sigma^2)},\;\forall t \ge 0.
$$
...
>**Lemm.** Let $X=(X_1,\ldots,X_k)$ be random vector with iid $1$-subGaussian coordinates. Then, there exist absolute constants $t_0,C>0$ such that for all $t\ge t_0$, it holds w.p $1-e^{-Ct^2k}$ that $\\|X\\| \le t\sqrt{k}$.

**Proof.** Note that $\|X\| = \sup_{x \in \mathbb S_{k-1}}|\langle X,x\rangle|$. Let $\epsilon \in (0, 1)$, to be prescribed later, and let $\enet$ be a maximal $\epsilon$-net for $\mathbb S_{k-1}$. For each $x \in \mathbb S_{N-1}$, there exists $z \in \enet$ such that $\\|x-z\\| \le \epsilon$. Writing $x = z + (x-z)$, one has

$$
\|X\| = \sup_{x \in \mathbb S_{k-1}}|\langle X,x\rangle| \le \max_{z \in \enet}|\langle X,z| + \sup_{x \in \mathbb S_{k-1},\;z \in \enet,\;\|x-z\| \le \epsilon}|\langle X,x-z\rangle
$$

We now state and prove one of the major tools for proving the main result.

>**Theorem.** There exists an absolute constant $C_0>0$ such that for every $t \ge C_0$, it holds w.p $1-e^{-C_1t^2N}$ that $s_{\max}(A) \le t\sqrt{N}$.

*Proof.* Let $\epsilon \in (0, 1)$, to be prescribed later, and let $\enet$ be a maximal $\epsilon$-net for $\mathbb S_{n-1}$. For every $x \in \mathbb S_{n-1}$, one can write $x = z + (x-z)$ for some $z \in \enet$ with $\\|x-z\\| \le \epsilon$. Thus, the triangle inequality gives $\\|Ax\\| \le \\|Az\\| + \\|A(x-z)\\| \le \\|Az\\| + \epsilon s_{\max}(A)$. Maximizing both sides and rearranging then gives

$$
s_{\max}(A) := \sup_{x \in \mathbb S_{n-1}}\|Ax\| \le (1-\epsilon)^{-1}\max_{z \in \enet}\|Az\|. \tag{1}
$$

Now, for every $z \in \mathbb S_{n-1}$, $Az = (a_1^Tz,\ldots,a_N^Tz)$ is a zero-mean random vector in $\mathbb R^N$ with iid coordinates which are each $1$-subGaussian (since $\\|z\\| = 1$). Therefore by well-known results (e.g see Vershynin book), we know that

>there exist absolute constants $C,C' > 0$ such that $\\|Az\\| \le u\sqrt{N}$ w.p $1-e^{-C'u^2N}$ for all $u \ge C$.

Thus, a simple union-bound gives

$$
\mathbb P(\max_{z \in \enet}\|Az\| \ge u\sqrt{N}) \le \card(\enet)\max_{z \in \enet}\mathbb P(\|Az\| \ge u\sqrt{N}) \le (3/\epsilon)^ne^{-C'u^2N} = ((3/\epsilon)^\lambda e^{-C'u^2})^N,
$$

and the result then follows from (1) by taking $\epsilon = 1/2$ (say!), and $u$ sifficiently large. $\quad\quad\qed$

We are now ready to prove the main result.

**Proof (main result).** Let $\epsilon \in (0,1)$, to be prescribed, and let $\enet$ be a maximal $\epsilon$-net for $\mathbb S_{n-1}$. For any $x \in \mathbb S_{n-1}$, there exists $z \in \enet$ such that $\|x-z\| \le \epsilon$. Now, by the triangle inequality, we have $\\|Ax\\| \ge \\|Az\\| - \\|A(x-z)\\|$. Minimizing both sides then gives
